...
!(function () {
  ...
  function add_dir_list(dir_list, dir_id = 0) {
    ...
    var list_dir_par = {
      url:
        api_url +
        `&path=${encodeURIComponent(path)}&recursion=${recursive ? 1 : 0}`,
      type: "GET",
      responseType: "json",
      onload: function (r) {
    ...
    if (file_id >= file_info_list.length) {
      ...
      file_info_list.forEach(function (item) {
        if (item.hasOwnProperty("errno")) {
          gen_failed_count++;
          failed_info += `<p>文件：${item.path}</p><p>失败原因：${checkErrno(
            item.errno
          )}(#${item.errno})</p>`;
        } else {
  ...
  function parse_download_data(r, file_id) {
    ...
    if (r.finalUrl.indexOf("issuecdn.baidupcs.com") !== -1) {
      ...
      if (file_md5) {
        ...
        download_file_data(
          file_id,
          pcs_url + `&path=${encodeURIComponent(file_info.path)}`
        );
        return;
      } else {
        if (!file_info.md5) {
        ...
        willOpen: () => {
          ...
          if (_dir) {
            ...
            btn.onclick = () => {
              location.href = `${
                location.origin
              }/disk/home?#/all?vmode=list&path=${encodeURIComponent(_dir)}`;
              Swal.close();
            };
            cBtn.before(btn);
          }
  ...
  const update_info = `<div class="panel-body" style="height: 250px; overflow-y:scroll">
        ...
        <p>若出现任何问题请前往 <a href="https://greasyfork.org/zh-CN/scripts/424574" rel="noopener noreferrer" target="_blank">发布页</a> 反馈</p>
